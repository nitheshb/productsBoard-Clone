    ...
    useEffect(() => {
        async function fetchFilterOptions() {
            ...
            //   }
                ...
                // const uniqueTeams = Array.from(new Set(teamData.map(item => item.team)))
                //     .filter(Boolean) as string[];
                
                // const uniqueStatuses = Array.from(new Set(statusData.map(item => item.status)))
                //     .filter(Boolean) as string[];
                
                // setAvailableTeams(uniqueTeams.length > 0 ? uniqueTeams : ["Frontend", "Backend", "Design", "QA", "DevOps"]);
                // setAvailableStatuses(uniqueStatuses.length > 0 ? uniqueStatuses : ["Todo", "In Progress", "Completed"]);
            } catch (error) {
                console.error("Error fetching filter options:", error);
