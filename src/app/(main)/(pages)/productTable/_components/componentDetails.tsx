...
// // //             throw new Error(errorData?.error || `Failed to fetch component (status ${response.status})`);
// // //           }
...
// // //   if (!component || !draftComponent) return <div>No component found</div>;
...
// //         setError(null);
// //         try {
// //           // Fix: Updated API endpoint path to match route handler
...
// //             <div className="mt-4 space-y-3">
// //               {(Object.keys(component) as (keyof ComponentType)[])
// //                 .filter(key => key !== 'id' && key !== 'name' && key !== 'features')
// //                 .map((key) => (
// //                 <div key={key} className="flex items-center gap-2">
// //                   <span className="text-gray-500 w-24">{key.charAt(0).toUpperCase() + key.slice(1).replace('_', ' ')}</span>
// //                   <div
